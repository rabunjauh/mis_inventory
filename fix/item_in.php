...
$now = date("Y-m-d");
$query = mysql_query("SELECT * FROM invoice_purchase");
		...
		for($i=0;$i<$count;$i++){
			...
			if($num_rows > 0){
				...
				if($num_rows_inventory == 0){
					$queryinsert = mysql_query("INSERT INTO inventory(item_id,inventory_quantity,alert_qtt,inventory_damage_qtt,inventory_added,inventory_update)
					VALUES('$itemid','$quantity','0','0','$now','$now')");
				}else{
